...
if st != 0:
    ...
    if st != 0:
        ...
        if v != total:
            send_text(agentid,'Warning: %s not enough reserved instance !!!' % k)
        else:
            end.sort()
            day = (datetime.datetime.strptime(end[0], TIMEFORMAT) - today).days
            if day == 1:
                send_text(agentid, 'Warning: %s reserved instance will expired !!!' % k)
